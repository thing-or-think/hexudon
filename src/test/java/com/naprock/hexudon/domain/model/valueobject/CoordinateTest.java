...
class CoordinateTest {
    ...
    void testConstructor_shouldThrowExceptionWhenNegative() {
        ...
        assertTrue(exceptionX.getMessage().contains("Coordinate x cannot be negative"));

        GameRuleViolationException exceptionY = assertThrows(GameRuleViolationException.class, () -> new Coordinate(2, -3));
        assertEquals(ErrorCode.VALIDATION_ERROR, exceptionY.getErrorCode());
        assertTrue(exceptionY.getMessage().contains("Coordinate y cannot be negative"));
    }
    ...
    void testDistanceTo_shouldCalculateCorrectHexDistance() {
        ...
        // (0,0) to (2,2) in Odd-R layout
        // Cube(0,0) = x=0, z=0, y=0
        // Cube(2,2) = x = 2 - 1 = 1, z = 2, y = -1 - 2 = -3
        // max(|1-0|, |-3-0|, |2-0|) = max(1, 3, 2) = 3
    ...
    void testGetNeighbor_shouldReturnCorrectNeighbor() {
        ...
        // Test neighbor in NORTHEAST
        // In Direction enum:
        // Even row: dy = -1, dx = 1 (assuming standard odd-r layout direction logic)
    ...
    void testToString() {
        ...
        assertEquals("(5, 7)", coordinate.toString());
    }
}
